...
const EditExpense = ({ expenseId, serverMode }: { expenseId: string; serverMode: Mode }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const errorMessage = error.response?.data?.message || error.message || 'Failed to load expense'
        ...
        <form onSubmit={handleSubmit}>
          <Grid container spacing={5}>
            ...
            <Grid size={{ xs: 12, md: 6 }}>
              ...
              >
                {formData.attachment ? formData.attachment.name : formData.existingAttachment ? 'Change Attachment' : 'Upload Receipt/Attachment'}
                <input
                  type='file'
                  hidden
              ...
              {formData.existingAttachment && !formData.attachment && (
                <Typography variant='caption' className='mts-2 block'>
                  Current: <a href={`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000'}/storage/${formData.existingAttachment}`} target='_blank' rel='noopener noreferrer'>View Attachment</a>
                </Typography>
              )}
              {formData.attachment && (
                <Typography variant='caption' className='mts-2 block'>
                  New: {formData.attachment.name}
                </Typography>
              )}
            </Grid>
            <Grid size={{ xs: 12 }}>
